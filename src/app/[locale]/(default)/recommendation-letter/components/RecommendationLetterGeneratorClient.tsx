  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const quotaRes = await fetch('/api/user/deduct-quota', {
        ...
        body: JSON.stringify({ function_type: 'recommendation-letter' }),
      });
      const quotaData = await quotaRes.json();
      if (quotaData.code !== 0) {
        toast.error(quotaData.message || '推荐信次数不足，请先购买套餐');
      ...
      router.push(`/${locale}/recommendation-letter/result/${document.uuid}?${resultParams.toString()}`);
    } catch (error) {
